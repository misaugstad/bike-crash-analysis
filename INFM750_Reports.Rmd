...
classes <- c("factor", "factor", "factor", "factor", "numeric", "numeric", replicate(4, "factor"),
             replicate(8, "numeric"), replicate(5, "factor"), "numeric", replicate(5, "factor"))
polio <- read.csv("data/NYPD_Motor_Vehicle_Collisions.csv",
                  skip=0, header=T, na.strings=c("-", ""), colClasses = classes)

# Fixed date and time columns to have appropriate data types
polio$TIME <- as.POSIXct(polio$TIME, format = "%H:%M")
polio$DATE <- as.Date(polio$DATE, format = "%m/%d/%Y")
