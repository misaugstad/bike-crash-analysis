...
```{r, echo=TRUE,warning=FALSE,message=FALSE,tidy=F,eval=T,fig.width=9}
pol <- crash.data.lon %>% group_by(BOROUGH, DATE) %>%  dplyr::summarise( c=sum(CYCLIST.CASUALTIES, na.rm=T))
...
output <- filter(crash.data.lon,CYCLIST.CASUALTIES>=1 , LATITUDE != "NA")
...
```{r, echo=TRUE,warning=FALSE,message=FALSE,tidy=F,eval=T,fig.width=9,fig.height=9}
output%>% select(1:2,5:6,30) %>%  group_by(DATE) %>% summarise( c=sum(CYCLIST.CASUALTIES, na.rm=T)) %>%  kable("html") %>%   kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r, echo=TRUE,warning=FALSE,message=FALSE,tidy=F,eval=T,fig.width=9,fig.height=9}
Foutput <-  output%>% select(1:2,5:6,30) %>%  group_by(DATE) %>% summarise( c=sum(CYCLIST.CASUALTIES, na.rm=T))
```
...
# timeOutput <- output  %>%  select(1:2,5:6,30)  %>%  summarise( c=sum(CYCLIST.CASUALTIES, na.rm=T))
